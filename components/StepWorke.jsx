...
export default function Component({ setSenddata }) {
  ...
  const handleNextStep = () => {
    ...
    if (
      currentStep === 4 &&
      (boxes.length === 0 || fontColor === null || isChanged)
    ) {
      setOpenAlert(true);
  ...
  return (
    <div className="mx-auto  w-1/2 mt-20 space-y-8 text-center">
      ...
      <p
        className="text-muted-foreground mt-2  mb-2 text-sm"
        role="region"
        aria-live="polite"
      >
        {currentStep === 1 &&
          "Enter the name of your event to begin setting up your invitation."}
        {currentStep === 2 && "Upload an Excel file with the list of invitees."}
        {currentStep === 3 && "Upload your invitation design template image."}
        {currentStep === 4 &&
          "Mark where the name and QR should appear on the design."}
      </p>
    </div>
  );
}
